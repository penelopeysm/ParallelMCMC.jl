...
# logp / gradlogp that work on any AbstractVector (CPU or GPU)
_logp_gpu(x)     = -0.5f0 * dot(x, x)
_gradlogp_gpu(x) = -x
...
@testset "DEER.solve on GPU with AutoEnzyme" begin
    ...
    # Build a minimal TapedRecursion directly (avoids interface overhead)
    ε    = 0.05f0
    tape = map(1:T) do _
    ...
    S, info = DEER.solve(rec, s0_gpu; maxiter=20, return_info=true)
...
@testset "DEERSampler interface on GPU" begin
    ...
    sampler = DEERSampler(0.05f0; T=8, maxiter=20, backend=ADTypes.AutoEnzyme())
